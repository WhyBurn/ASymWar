...
public class CustomSpaceInspector : PropertyDrawer
{
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        Rect spaceRect = new Rect(position.x, position.y, width / 10 * 5, height);
        Rect xRect = new Rect(position.x + width / 10 * 5, position.y, width / 10, height);
        Rect yRect = new Rect(position.x + width / 10 * 6, position.y, width / 10, height);
        Rect idRect = new Rect(position.x + width / 10 * 7, position.y, width / 10, height);
        Rect recruitRect = new Rect(position.x + width / 10 * 8, position.y, width / 10, height);
        Rect nameRect = new Rect(position.x + width / 10 * 9, position.y, width / 10, height);
